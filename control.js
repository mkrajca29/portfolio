...
let peristaltic = {
    ...
    sources: [
        "./img/Persitaltic_01.png",
        "./img/Persitaltic_02.png",
        "./img/Peristaltic_realizace.png"
    ],
    text: [
        "Konstrukční verze v1",
        "Konstrukční verze v2",
        "Realizace první verze"
    ],
    popis: [
        ...
        "Druhá verze čerpadla byla navržena s celistvým zakrytováním. Design této verze se vyznačuje tvarově komplikovanějšími křivky.",
        "Rám první verze navrženého čerpadla byl vyroben pomocí technologie přesného lití do skořepinové formy. Jednotlivé komponenty jsou pak nakupovány, či výrobkem z 3D tiskárny."
    ]
}
let bladeless = {
    ...
    sources: [
        "./img/Bladeless_01.png",
        "./img/Bladeless_02.png",
        "./img/Bladeless.png"
    ],
    text: [
    ...
    popis: [
        ...
        "Na obrázku lze vidět řez ventilátorem, kde je k vidění stator a rotor pohánějící elektromotor. Vzduch je aerodynamicky usměrněn do prstence, který představuje vzduchový tunel. ",
        "Výsledný funkční ventilátor byl zhotoven pomocí 3D tisku metodou FDM. "
    ]
}
let others = {
    ...
    popis: [
        "Zadáním mé bakalářské práce bylo navrhnout malou stolní kotoučovou pilu pro dřevozpracující průmysl. Pila je vybavena mechanismy pro naklápění a zvedání kotouče. Z technologického hlediska koncept kombinuje vstřikovaný plast a hliník.",
        "Zadáním mé diplomové práce bylo vytvořit konstrukční řešení vakuového zařízení pro lití do silikonových forem. Toto zařízení je rozdělené do dvou základních prostorů, kdy v horním se odehrává samotný odlévací proces a ve spodním je umístěná forma."
    ]
}
let webdev = {
    ...
    popis: [
        ...
        "Samozřejmostí je responzivní design, který se přizpůsobí všem typům zařízení."
    ]
}
let canvas_game = {
    ...
    popis: [
        ...
        "Kromě závodění samotného lze měnit barvu vozidla. Za vyhrané peníze je pak možno nakoupit díly, které zvýší pravděpodobnost výhry. Tzn. generovaná hodnota rychlosti je vyšší než dříve."
    ]
}
let CVweb = {
    ...
    text: [
        "Resume Builder",
        "Resume Builder - náhledová část"
    ],
    popis: [
        "Jedním z nejnovějších projektů, na kterém pracuji je tzv. Resume Builder. Aplikace je rozdělena na editační část a náhledovou část. Tato aplikace je vyvíjena pomocí frameworku Vue.js.",
        "Samotná náhledová část není určena k editaci. Slouží pouze k detailnímu prohlížení a přiblížení CV. Po kompletním vyplnění bude možné životopis stáhnout v .PDF formátu."
    ]
}
...
let projecttext_zoom = document.querySelector("#imgPopis"); 
let big_Image = document.querySelector("#bigImage");
let imgText = document.querySelector("#imgText");

let getImgNodeList = document.querySelectorAll(".preview img");
let poleImg = Array.prototype.slice.call(getImgNodeList);

let getInfoNodeList = document.querySelectorAll(".projectInfo");
let poleInfo = Array.prototype.slice.call(getInfoNodeList);

let projecttext_NodeList = document.querySelectorAll(".projecttext");
let projecttext = Array.prototype.slice.call(projecttext_NodeList);

let actualImage;
let getIndex;


let rank_node = document.querySelectorAll(".rank");
let rank_value = document.querySelectorAll(".rank_value");
let rank_array = Array.prototype.slice.call(rank_node);
let rank_value_array = Array.prototype.slice.call(rank_value);

let html_val = 85, css_val = 70, js_val = 60, vuejs = 40, git_val = 55, nodejs = 25;
...
function nextImg(object,index) {
    ...
    if (object.click_index >= object.sources.length) {object.click_index=0}
...
function prevImg(object,index) {
    ...
    if (object.click_index == 0) {object.click_index = object.sources.length}
    object.click_index--;
...
let elementzoom = document.querySelector("#imagepreview");
...
function imagezoom(object) {
    ...
    let sirka = window.screen.width;
    /* console.log(sirka) */
    let imageElement = document.querySelector("#bigImage");
    ...
    if(!detectIE()) {
        if (sirka >= 1040) {
            ...
            if (object.identificator == "peristaltic") {
                ...
                let bladeless_settings = document.querySelector("#bigImage");
